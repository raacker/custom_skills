...
<task>
Take a one-line startup idea and produce a unified business_analysis.md that (a) survives PG-grade fatal-flaw scrutiny, (b) proves the problem is a painkiller with a specific named early adopter, and (c) maps the real enemy including current behavior. Output is structured to feed directly into the obra/superpowers brainstorming skill via the §5 Handoff section.
</task>
